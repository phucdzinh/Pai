...
class VietnameseEngine:
    def __init__(self, input_method='TELEX', charset='UNICODE'):
        ...
        if self.input_method == 'TELEX':
            self.tones = TELEX_TONES
            self.modifiers = TELEX_MODIFIERS
        elif self.input_method == 'VNI':
            self.tones = VNI_TONES
            self.modifiers = VNI_MODIFIERS
        elif self.input_method == 'VIQR':
            self.tones = VIQR_TONES
            self.modifiers = VIQR_MODIFIERS
        else:
            self.tones = {}
            self.modifiers = set()
    ...
    def apply_modifier_viqr(self, chars, mod_key):
        if mod_key == '-':
            for i in range(len(chars)-1, -1, -1):
                c_low = chars[i].lower()
                if c_low == 'd': chars[i] = 'Đ' if chars[i].isupper() else 'đ'; return True
        ...
        for char in raw_buffer:
            ...
            if c_low in self.tones:
                ...
                if self.input_method == 'TELEX':
                    ...
                    state = self.apply_modifier_viqr(result_chars, c_low)
    ...
    def on_press(self, key):
        ...
        try:
            ...
            if char is not None:
                if self.input_method == 'VNI' and char.isdigit():
                    pass
                elif self.input_method == 'VIQR' and (char in self.tones or char in self.modifiers):
                    pass
